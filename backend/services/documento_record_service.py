...
async def create_documento_record(
    usuario_id: str,
    titulo: str,
    archivos: list[str],
    ...
    """
    return await documento_mutations_repo.create(
        usuario_id, titulo, archivos, plantilla_nombre, secciones, indicaciones, opciones,
    )


async def list_all_documentos() -> list[dict]:
    """Retorna todos los registros de documentos. Solo para administradores."""
    return await documento_repo.find_all()


async def list_user_documentos(user_id: str) -> list[dict]:
    """Retorna los registros de documentos del usuario indicado."""
    return await documento_repo.find_by_user(user_id)
...
async def get_documento_by_id(
    ...
    """
    doc = await documento_repo.find_by_id(documento_id)
    if not doc:
