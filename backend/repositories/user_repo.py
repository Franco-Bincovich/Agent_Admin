...
async def find_by_email(email: str) -> dict | None:
    ...
    response = await asyncio.to_thread(
        lambda: get_supabase().table(_TABLE).select("*").eq("email", email).execute()
    )
...
async def find_by_id(user_id: str) -> dict | None:
    ...
    response = await asyncio.to_thread(
        lambda: get_supabase().table(_TABLE).select("*").eq("id", user_id).execute()
    )
    return response.data[0] if response.data else None
...
async def find_all() -> list[dict]:
    ...
    response = await asyncio.to_thread(
        lambda: get_supabase().table(_TABLE).select("*").order("creado_en", desc=True).execute()
    )
